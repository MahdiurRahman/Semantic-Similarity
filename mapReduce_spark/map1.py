...
documents_rdd = sc.parallelize(inFile.map(lambda line: line.split(' ')).collect())
...
term_frequency = documents_rdd.map(lambda document: processDocument_tf(document)).collect()
